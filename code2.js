...
class BezierEngine {
    ...
    static buildArcLUT(seg, steps = 200) {
        if (seg.type === "LINE") {
            return [
                { t: 0, arcLen: 0 },
    ...
    static tFromArcLength(lut, targetLen) {
        const last = lut[lut.length - 1];
        if (targetLen <= 0)
            return 0;
        if (targetLen >= last.arcLen)
            return 1;
    ...
    static calculateLength(p0, p1, p2, p3, type, steps = 200) {
        if (type === "LINE") {
 ...
 */
function placeClone(clone, cx, cy, angleDeg, rotate) {
    if (rotate) {
...
figma.ui.onmessage = (msg) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (msg.type === "generate") {
        ...
        const shapeNode = (yield figma.getNodeByIdAsync(msg.shapeId));
        const pathNode = (yield figma.getNodeByIdAsync(msg.pathId));
        ...
        for (const seg of network.segments) {
            ...
            const tStart = (_a = seg.tangentStart) !== null && _a !== void 0 ? _a : { x: 0, y: 0 };
            const tEnd = (_b = seg.tangentEnd) !== null && _b !== void 0 ? _b : { x: 0, y: 0 };
            ...
            const type = isLine ? "LINE" : "CUBIC";
            // Temporary segment (without lut) to build lut
            const tmp = { p0, p1, p2, p3, type, length: 0, lut: [] };
        ...
        for (let i = 0; i < count; i++) {
            ...
            if (targetDist > totalLength + 0.5)
                break;
            // Binary search for the correct segment
            let lo = 0, hi = segments.length - 1;
            while (lo < hi) {
                const mid = (lo + hi + 1) >> 1;
                if (cumLengths[mid] <= targetDist)
                    lo = mid;
                else
                    hi = mid - 1;
            }
        ...
        if (clones.length > 0) {
            ...
            figma.notify("No instances were placed. Check count / gap vs. path length.", { error: true });
        }
        figma.ui.postMessage({ type: "done" });
    }
});
